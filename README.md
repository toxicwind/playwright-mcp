...
Go to `Advanced settings` -> `Extensions` -> `Add custom extension`. Name to your liking, use type `STDIO`, and set the `command` to `npx @toxicwind/playwright-mcp`. Click "Add Extension".
</details>
...
import { createConnection } from '@toxicwind/playwright-mcp';
